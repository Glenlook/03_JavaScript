const user = document.querySelector("#use"); // 유저 이미지
const com = document.querySelector("#com");   // 컴퓨터 이미지

// 가위 바위 보 버튼들
const userBtn = document.querySelectorAll("#btn-tab > button"); 
...
// 게임 히스토리
const gameHistory = document.querySelector("#gamelog");
...
for(let i = 0 ; i < userBtn.length; i++){
  ...
  userBtn[i].addEventListener("click", function(){
    ...
    if(userChoice === comChoice){ // 같은 경우 === 비김
      ...
      gameHistory.innerHTML
        += `<li> <span id="draw">DRAW</span> ${str} </li>`;
    }
    ...
    // user가 이긴 경우
      ...
      gameHistory.innerHTML
        += `<li> <span id="win">WIN</span> ${str} </li>`;  
      ...
      gameHistory.innerHTML
        += `<li> <span id="lose">LOSE</span> ${str} </li>`;  
    }
