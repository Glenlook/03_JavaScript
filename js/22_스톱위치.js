...
// [0]:분 / [1]:초 / [2]:1/100초
const timelapse = document.querySelectorAll("#display > span");
...
function output(){
  ...
  // -> 1분 증가
  if(minute != timelapse[0].innerText){
  timelapse[0].innerText = minute;
  }

  // second에 저장된 값과 화면에 출력된 "초"가 다른 경우
  // -> 1초 증가
  if(second != timelapse[1].innerText){
  timelapse[1].innerText = second;
  }

  if(ms != timelapse[2].innerText){
  timelapse[2].innerText = ms;
  }
}
