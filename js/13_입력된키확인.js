// 준비한 이미지 파일명을 저장할 배열
const images = ["쿼카1.gif", "쿼카2.gif", "쿼카3.gif", "쿼카4.gif"];

// 클래스가 btn인 요소를 모두 얻어와 배열형태로 저장
...
document.addEventListener("keydown", function(e){
  ...
  // 배경을 이미지로 지정
  btns[index].style.backgroundImage = `url("../images/13_입력된키확인/${images[index]}")`;

  // 글자색 투명으로 변경
  // - rgba(red, green, blue, alpha)
  // - rgb 값은 0 ~ 255 / alpha(투명도) 0(투명) ~ 1(불투명)
  btns[index].style.color = "rgba(0, 0, 0, 0)";
});
...
document.addEventListener("keyup", function(e){
  ...
  // 배경 이미지 제거
  btns[index].style.backgroundImage = "none";

  // 글자색 검정
  btns[index].style.color = "rgba(0, 0, 0, 1)";
});
